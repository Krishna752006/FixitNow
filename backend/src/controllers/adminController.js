...
export const getAllUsers = async (req, res) => {
  try {
    ...
    if (!userType || userType === 'user') {
      const userQuery = { ...searchQuery, userType: 'user' };
      if (isVerified !== undefined) userQuery.isVerified = isVerified === 'true';
      
      const regularUsers = await User.find(userQuery)
        .select('-password')
        .sort({ createdAt: -1 });
    ...
    if (!userType || userType === 'professional') {
      const professionalQuery = { ...searchQuery };
      if (isVerified !== undefined) professionalQuery.isVerified = isVerified === 'true';
      
      const professionals = await Professional.find(professionalQuery)
        .select('-password')
        .sort({ createdAt: -1 });
      
      users = users.concat(professionals);
    }
    ...
    if (!userType || userType === 'professional') {
      const professionalQuery = { ...searchQuery };
      if (isVerified !== undefined) professionalQuery.isVerified = isVerified === 'true';
      total += await Professional.countDocuments(professionalQuery);
    }

    // Apply pagination
    const paginatedUsers = users.slice(skip, skip + pageLimit);
...
export const getTransactionHistory = async (req, res) => {
  try {
    ...
    const transactions = await Job.find({ status: 'completed' })
      .select('_id finalPrice createdAt user professional')
