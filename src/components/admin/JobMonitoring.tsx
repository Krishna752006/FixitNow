...
interface Job {
  ...
  professional?: { firstName: string; lastName: string; email: string; phone: string };
  location: {
    address: string;
    city: string;
    state: string;
...
const JobMonitoring: React.FC = () => {
  ...
  const fetchJobs = async () => {
    try {
      ...
      if (response.success) {
        console.log('📊 Jobs fetched:', response.data.length, 'Total:', response.pagination.total);
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div>
                          ...
                          <p className="text-gray-900 font-medium">
                            {job.user?.firstName || 'N/A'} {job.user?.lastName || ''}
                          </p>
                          <p className="text-gray-600 text-xs">{job.user?.email || 'N/A'}</p>
                        </div>
                        <div>
                          ...
                          {job.professional && (
                            <p className="text-gray-600 text-xs">{job.professional.email}</p>
                          )}
                        </div>
                        <div>
                          <p className="text-gray-500 text-xs font-medium">LOCATION</p>
                          <p className="text-gray-900 font-medium">{job.location?.city || 'N/A'}</p>
                          <p className="text-gray-600 text-xs">{job.location?.state || 'N/A'}</p>
                        </div>
                        <div>
                        ...
                        <div className="space-y-4 text-sm">
                          ...
                          <div>
                            ...
                            <div className="bg-white p-3 rounded border border-gray-200">
                              ...
                              <p className="text-gray-600">{job.user.phone}</p>
                            </div>
                          </div>
                          {job.professional && (
                            <div>
                              ...
                              <div className="bg-white p-3 rounded border border-gray-200">
                                ...
                                <p className="text-gray-600">{job.professional.email}</p>
                                <p className="text-gray-600">{job.professional.phone}</p>
                              </div>
                            </div>
                          )}
                        </div>
